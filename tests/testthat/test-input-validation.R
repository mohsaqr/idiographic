test_that("min_obs is validated as a whole number >= 1", {
  ...
  expect_error(graphical_var(d, vars = c("A", "B", "C"), id = "id",
...
test_that("build_mlvar / build_gimme validate their column args", {
  d <- synth_panel(n_id = 6, days = 2, beeps = 12)
  expect_error(
    build_mlvar(d, vars = c("A", "B", "C"), id = "id", day = "missing"),
    "not found"
  )
  skip_if_not_installed("lavaan")
  expect_error(
    build_gimme(d, vars = c("A", "B", "C"), id = "id", beep = c("x", "y")),
    "single column name"
  )
})
