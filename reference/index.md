# Package index

## All functions

- [`as_netobject()`](https://mohsaqr.github.io/idiographic/reference/as_netobject.md)
  : Coerce to a netobject

- [`as_netobject(`*`<gvar_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/as_netobject.gvar_result.md)
  : Coerce a gvar_result to plottable netobjects

- [`as_netobject(`*`<net_gimme>`*`)`](https://mohsaqr.github.io/idiographic/reference/as_netobject.net_gimme.md)
  : Plottable netobject(s) from a GIMME fit

- [`as_netobject(`*`<net_mlvar>`*`)`](https://mohsaqr.github.io/idiographic/reference/as_netobject.net_mlvar.md)
  : Plottable netobjects from an mlVAR fit

- [`as_netobject(`*`<var_bayes_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/as_netobject.var_bayes_result.md)
  : Coerce a var_bayes_result to plottable netobjects

- [`audit_preprocess()`](https://mohsaqr.github.io/idiographic/reference/audit_preprocess.md)
  : Audit preprocessing and lag construction

- [`build_gimme()`](https://mohsaqr.github.io/idiographic/reference/build_gimme.md)
  : GIMME: Group Iterative Multiple Model Estimation

- [`build_mlvar()`](https://mohsaqr.github.io/idiographic/reference/build_mlvar.md)
  : Build a Multilevel Vector Autoregression (mlVAR) network

- [`build_mlvar_bayes()`](https://mohsaqr.github.io/idiographic/reference/build_mlvar_bayes.md)
  : Build a Bayesian multilevel VAR network (Mplus DSEM equivalent)

- [`build_mlvar_mplus()`](https://mohsaqr.github.io/idiographic/reference/build_mlvar_mplus.md)
  : Build an Mplus-backed multilevel VAR network

- [`build_usem()`](https://mohsaqr.github.io/idiographic/reference/build_usem.md)
  : Build a user-specified unified SEM network

- [`build_var()`](https://mohsaqr.github.io/idiographic/reference/build_var.md)
  : Build an ordinary least-squares VAR network

- [`build_var_bayes()`](https://mohsaqr.github.io/idiographic/reference/build_var_bayes.md)
  : Build a Bayesian VAR(1) network (unregularized Mplus-equivalent)

- [`build_var_each()`](https://mohsaqr.github.io/idiographic/reference/build_var_each.md)
  : Fit an ordinary least-squares VAR for every subject

- [`coefs()`](https://mohsaqr.github.io/idiographic/reference/coefs.md)
  : Tidy coefficients from a fitted mlvar model

- [`compare_idiographic()`](https://mohsaqr.github.io/idiographic/reference/compare_idiographic.md)
  : Compare idiographic estimators on one dataset

- [`edges()`](https://mohsaqr.github.io/idiographic/reference/edges.md)
  : Tidy edge table for any idiographic result

- [`estimate_stability()`](https://mohsaqr.github.io/idiographic/reference/estimate_stability.md)
  : Estimate edge stability by block resampling (experimental)

- [`extract_edges()`](https://mohsaqr.github.io/idiographic/reference/extract_edges.md)
  : Tidy edge table from a network object

- [`graphical_var()`](https://mohsaqr.github.io/idiographic/reference/graphical_var.md)
  : Graphical VAR Estimation

- [`graphical_var_each()`](https://mohsaqr.github.io/idiographic/reference/graphical_var_each.md)
  : Fit a graphical VAR for every subject

- [`matrices()`](https://mohsaqr.github.io/idiographic/reference/matrices.md)
  : Print model matrices for idiographic results

- [`nodes()`](https://mohsaqr.github.io/idiographic/reference/nodes.md)
  : Tidy per-node strength table for any idiographic result

- [`plot_gimme()`](https://mohsaqr.github.io/idiographic/reference/plot_gimme.md)
  :

  Faithful GIMME network plot (the `gimme`-package convention, via
  cograph)

- [`plot(`*`<var_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<gvar_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<var_bayes_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<net_mlvar>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<net_usem>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<net_gimme>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<var_list>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<gvar_list>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<rolling_var_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<rolling_gvar_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<stability_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/plot_idiographic.md)
  : Plot an idiographic network result

- [`print(`*`<forecast_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.forecast_result.md)
  : Print method for forecast validation results

- [`print(`*`<gvar_list>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.gvar_list.md)
  : Print a list of per-subject graphical VARs

- [`print(`*`<gvar_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.gvar_result.md)
  : Print Method for gvar_result

- [`print(`*`<model_comparison>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.model_comparison.md)
  : Print method for model comparisons

- [`print(`*`<net_gimme>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.net_gimme.md)
  : Print Method for net_gimme

- [`print(`*`<net_mlvar>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.net_mlvar.md)
  : Print method for net_mlvar

- [`print(`*`<net_mlvar_bayes>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.net_mlvar_bayes.md)
  : Print method for net_mlvar_bayes

- [`print(`*`<net_usem>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.net_usem.md)
  : Print method for uSEM fits

- [`print(`*`<preprocess_audit>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.preprocess_audit.md)
  : Print method for preprocessing audits

- [`print(`*`<rolling_gvar_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.rolling_gvar_result.md)
  : Print method for rolling graphical VAR results

- [`print(`*`<rolling_var_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.rolling_var_result.md)
  : Print method for rolling VAR results

- [`print(`*`<stability_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.stability_result.md)
  : Print method for stability results

- [`print(`*`<var_bayes_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.var_bayes_result.md)
  : Print method for var_bayes_result

- [`print(`*`<var_list>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.var_list.md)
  : Print a list of per-subject ordinary VARs

- [`print(`*`<var_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/print.var_result.md)
  : Print method for ordinary VAR fits

- [`rolling_graphical_var()`](https://mohsaqr.github.io/idiographic/reference/rolling_graphical_var.md)
  : Estimate rolling-window graphical VAR networks

- [`rolling_var()`](https://mohsaqr.github.io/idiographic/reference/rolling_var.md)
  : Estimate rolling-window ordinary VAR networks

- [`srl`](https://mohsaqr.github.io/idiographic/reference/srl.md) :
  Self-regulated learning intensive longitudinal data (Chapter 20)

- [`summary(`*`<gvar_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/summary.gvar_result.md)
  : Summary Method for gvar_result

- [`summary(`*`<net_gimme>`*`)`](https://mohsaqr.github.io/idiographic/reference/summary.net_gimme.md)
  : Summary Method for net_gimme

- [`summary(`*`<net_mlvar>`*`)`](https://mohsaqr.github.io/idiographic/reference/summary.net_mlvar.md)
  : Summary method for net_mlvar

- [`summary(`*`<net_usem>`*`)`](https://mohsaqr.github.io/idiographic/reference/summary.net_usem.md)
  : Summary method for uSEM fits

- [`summary(`*`<var_bayes_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/summary.var_bayes_result.md)
  : Summary method for var_bayes_result

- [`summary(`*`<var_result>`*`)`](https://mohsaqr.github.io/idiographic/reference/summary.var_result.md)
  : Summary method for ordinary VAR fits

- [`validate_forecast()`](https://mohsaqr.github.io/idiographic/reference/validate_forecast.md)
  : Validate one-step forecasts from idiographic VAR models
  (experimental)
