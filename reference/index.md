# Package index

## All functions

- [`argument_coverage()`](https://pak.dynasite.org/idiographic/reference/argument_coverage.md)
  : Argument-by-argument validation coverage

- [`as.data.frame(`*`<glasso_path_result>`*`)`](https://pak.dynasite.org/idiographic/reference/as.data.frame.glasso_path_result.md)
  : Tidy a graphical-lasso path

- [`as.data.frame(`*`<glasso_result>`*`)`](https://pak.dynasite.org/idiographic/reference/as.data.frame.glasso_result.md)
  : Tidy a graphical-lasso fit

- [`as_netobject()`](https://pak.dynasite.org/idiographic/reference/as_netobject.md)
  : Coerce to a netobject

- [`as_netobject(`*`<gvar_result>`*`)`](https://pak.dynasite.org/idiographic/reference/as_netobject.gvar_result.md)
  : Coerce a gvar_result to plottable netobjects

- [`as_netobject(`*`<net_gimme>`*`)`](https://pak.dynasite.org/idiographic/reference/as_netobject.net_gimme.md)
  : Plottable netobject(s) from a GIMME fit

- [`as_netobject(`*`<net_mlvar>`*`)`](https://pak.dynasite.org/idiographic/reference/as_netobject.net_mlvar.md)
  : Plottable netobjects from an mlVAR fit

- [`as_netobject(`*`<var_bayes_result>`*`)`](https://pak.dynasite.org/idiographic/reference/as_netobject.var_bayes_result.md)
  : Coerce a var_bayes_result to plottable netobjects

- [`coefs()`](https://pak.dynasite.org/idiographic/reference/coefs.md) :
  Tidy coefficients from a fitted mlvar model

- [`compare_idiographic()`](https://pak.dynasite.org/idiographic/reference/compare_idiographic.md)
  : Compare idiographic estimators on one dataset

- [`edges()`](https://pak.dynasite.org/idiographic/reference/edges.md) :
  Tidy edge table for any idiographic result

- [`equivalence()`](https://pak.dynasite.org/idiographic/reference/equivalence.md)
  : Report method-equivalence evidence

- [`equivalence_table()`](https://pak.dynasite.org/idiographic/reference/equivalence_table.md)
  : Package-wide equivalence and validation ledger

- [`esm_srl`](https://pak.dynasite.org/idiographic/reference/esm_srl.md)
  : Momentary self-regulated-learning experience-sampling data

- [`estimate_stability()`](https://pak.dynasite.org/idiographic/reference/estimate_stability.md)
  : Estimate edge stability by block resampling (experimental)

- [`estimator_info()`](https://pak.dynasite.org/idiographic/reference/estimator_info.md)
  : Inspect a registered estimator

- [`extract_edges()`](https://pak.dynasite.org/idiographic/reference/extract_edges.md)
  : Tidy edge table from a network object

- [`fit_gimme()`](https://pak.dynasite.org/idiographic/reference/fit_gimme.md)
  : GIMME: Group Iterative Multiple Model Estimation

- [`fit_graphical_var()`](https://pak.dynasite.org/idiographic/reference/fit_graphical_var.md)
  : Graphical VAR Estimation

- [`fit_graphical_var_each()`](https://pak.dynasite.org/idiographic/reference/fit_graphical_var_each.md)
  : Fit a graphical VAR for every subject

- [`fit_idiographic()`](https://pak.dynasite.org/idiographic/reference/fit_idiographic.md)
  : Fit an idiographic model through the unified interface

- [`fit_ml()`](https://pak.dynasite.org/idiographic/reference/fit_ml.md)
  [`fit_idiographic_ml()`](https://pak.dynasite.org/idiographic/reference/fit_ml.md)
  [`fit_individualized_ml()`](https://pak.dynasite.org/idiographic/reference/fit_ml.md)
  : Fit idiographic supervised machine-learning models

- [`fit_mlvar()`](https://pak.dynasite.org/idiographic/reference/fit_mlvar.md)
  : Build a Multilevel Vector Autoregression (mlVAR) network

- [`fit_mlvar_bayes()`](https://pak.dynasite.org/idiographic/reference/fit_mlvar_bayes.md)
  : Build a Bayesian multilevel VAR network (Mplus DSEM-targeted)

- [`fit_mlvar_mplus()`](https://pak.dynasite.org/idiographic/reference/fit_mlvar_mplus.md)
  : Build an Mplus-backed multilevel VAR network

- [`fit_rolling_graphical_var()`](https://pak.dynasite.org/idiographic/reference/fit_rolling_graphical_var.md)
  : Estimate rolling-window graphical VAR networks

- [`fit_rolling_var()`](https://pak.dynasite.org/idiographic/reference/fit_rolling_var.md)
  : Estimate rolling-window ordinary VAR networks

- [`fit_usem()`](https://pak.dynasite.org/idiographic/reference/fit_usem.md)
  : Build a user-specified unified SEM network

- [`fit_var()`](https://pak.dynasite.org/idiographic/reference/fit_var.md)
  : Build an ordinary least-squares VAR network

- [`fit_var_bayes()`](https://pak.dynasite.org/idiographic/reference/fit_var_bayes.md)
  : Build a Bayesian VAR(1) network (unregularized, Mplus-targeted)

- [`fit_var_each()`](https://pak.dynasite.org/idiographic/reference/fit_var_each.md)
  : Fit an ordinary least-squares VAR for every subject

- [`get_estimator()`](https://pak.dynasite.org/idiographic/reference/get_estimator.md)
  : Get a registered estimator function

- [`glasso_fit()`](https://pak.dynasite.org/idiographic/reference/glasso_fit.md)
  : Fit a graphical lasso at a fixed penalty

- [`glasso_kkt()`](https://pak.dynasite.org/idiographic/reference/glasso_kkt.md)
  : Certify a graphical-lasso solution from its optimality conditions

- [`glasso_path()`](https://pak.dynasite.org/idiographic/reference/glasso_path.md)
  : Fit a graphical lasso over a path of penalties

- [`idiographic`](https://pak.dynasite.org/idiographic/reference/idiographic-package.md)
  [`idiographic-package`](https://pak.dynasite.org/idiographic/reference/idiographic-package.md)
  : idiographic: Idiographic Network Estimation from Intensive
  Longitudinal Data

- [`list_estimators()`](https://pak.dynasite.org/idiographic/reference/list_estimators.md)
  : Registered idiographic estimators

- [`matrices()`](https://pak.dynasite.org/idiographic/reference/matrices.md)
  : Print model matrices for idiographic results

- [`nodes()`](https://pak.dynasite.org/idiographic/reference/nodes.md) :
  Tidy per-node strength table for any idiographic result

- [`plot_gimme()`](https://pak.dynasite.org/idiographic/reference/plot_gimme.md)
  :

  Faithful GIMME network plot (the `gimme`-package convention, via
  cograph)

- [`plot(`*`<var_result>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<gvar_result>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<var_bayes_result>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<net_mlvar>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<net_usem>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<net_gimme>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<var_list>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<gvar_list>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<rolling_var_result>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<rolling_gvar_result>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  [`plot(`*`<stability_result>`*`)`](https://pak.dynasite.org/idiographic/reference/plot_idiographic.md)
  : Plot an idiographic network result

- [`predict(`*`<idioml_result>`*`)`](https://pak.dynasite.org/idiographic/reference/predict.idioml_result.md)
  : Predict from an idiographic ML result

- [`preprocess()`](https://pak.dynasite.org/idiographic/reference/preprocess.md)
  : Preprocess and audit idiographic time-series data

- [`print(`*`<forecast_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.forecast_result.md)
  : Print method for forecast validation results

- [`print(`*`<glasso_path_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.glasso_path_result.md)
  : Print a graphical-lasso path

- [`print(`*`<glasso_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.glasso_result.md)
  : Print a graphical-lasso fit

- [`print(`*`<gvar_list>`*`)`](https://pak.dynasite.org/idiographic/reference/print.gvar_list.md)
  : Print a list of per-subject graphical VARs

- [`print(`*`<gvar_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.gvar_result.md)
  : Print Method for gvar_result

- [`print(`*`<idioml_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.idioml_result.md)
  : Print method for idiographic ML fits

- [`print(`*`<model_comparison>`*`)`](https://pak.dynasite.org/idiographic/reference/print.model_comparison.md)
  : Print method for model comparisons

- [`print(`*`<net_gimme>`*`)`](https://pak.dynasite.org/idiographic/reference/print.net_gimme.md)
  : Print Method for net_gimme

- [`print(`*`<net_mlvar>`*`)`](https://pak.dynasite.org/idiographic/reference/print.net_mlvar.md)
  : Print method for net_mlvar

- [`print(`*`<net_mlvar_bayes>`*`)`](https://pak.dynasite.org/idiographic/reference/print.net_mlvar_bayes.md)
  : Print method for net_mlvar_bayes

- [`print(`*`<net_usem>`*`)`](https://pak.dynasite.org/idiographic/reference/print.net_usem.md)
  : Print method for uSEM fits

- [`print(`*`<preprocess_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.preprocess_result.md)
  : Print method for preprocessing results

- [`print(`*`<rolling_gvar_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.rolling_gvar_result.md)
  : Print method for rolling graphical VAR results

- [`print(`*`<rolling_var_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.rolling_var_result.md)
  : Print method for rolling VAR results

- [`print(`*`<stability_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.stability_result.md)
  : Print method for stability results

- [`print(`*`<var_bayes_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.var_bayes_result.md)
  : Print method for var_bayes_result

- [`print(`*`<var_list>`*`)`](https://pak.dynasite.org/idiographic/reference/print.var_list.md)
  : Print a list of per-subject ordinary VARs

- [`print(`*`<var_result>`*`)`](https://pak.dynasite.org/idiographic/reference/print.var_result.md)
  : Print method for ordinary VAR fits

- [`register_estimator()`](https://pak.dynasite.org/idiographic/reference/register_estimator.md)
  : Register an idiographic estimator or workflow

- [`remove_estimator()`](https://pak.dynasite.org/idiographic/reference/remove_estimator.md)
  : Remove a registered estimator

- [`srl`](https://pak.dynasite.org/idiographic/reference/srl.md) :
  Self-regulated learning intensive longitudinal data (Chapter 20)

- [`summary(`*`<gvar_result>`*`)`](https://pak.dynasite.org/idiographic/reference/summary.gvar_result.md)
  : Summary Method for gvar_result

- [`summary(`*`<idioml_result>`*`)`](https://pak.dynasite.org/idiographic/reference/summary.idioml_result.md)
  : Summary method for idiographic ML fits

- [`summary(`*`<net_gimme>`*`)`](https://pak.dynasite.org/idiographic/reference/summary.net_gimme.md)
  : Summary Method for net_gimme

- [`summary(`*`<net_mlvar>`*`)`](https://pak.dynasite.org/idiographic/reference/summary.net_mlvar.md)
  : Summary method for net_mlvar

- [`summary(`*`<net_usem>`*`)`](https://pak.dynasite.org/idiographic/reference/summary.net_usem.md)
  : Summary method for uSEM fits

- [`summary(`*`<preprocess_result>`*`)`](https://pak.dynasite.org/idiographic/reference/summary.preprocess_result.md)
  : Summary method for preprocessing results

- [`summary(`*`<var_bayes_result>`*`)`](https://pak.dynasite.org/idiographic/reference/summary.var_bayes_result.md)
  : Summary method for var_bayes_result

- [`summary(`*`<var_result>`*`)`](https://pak.dynasite.org/idiographic/reference/summary.var_result.md)
  : Summary method for ordinary VAR fits

- [`validate_forecast()`](https://pak.dynasite.org/idiographic/reference/validate_forecast.md)
  : Validate one-step forecasts from idiographic VAR models
  (experimental)
