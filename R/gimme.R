...
#' @param group_correct Character. Group-level multiple-comparison correction;
#'   only \code{"Bonferoni Group"} (the \code{gimme} default) is implemented.
...
#'   \item{\code{psi}}{List of per-person \code{2p x 2p} residual covariance
#'     matrices over \code{c(lag_names, varnames)} (the standardized lavaan
#'     psi block), not the \code{p x p} current-variable block alone.}
...
#' }
#'
#' @seealso \code{\link{build_mlvar}}, \code{\link{graphical_var}},
...
build_gimme <- function(data,
  ...
  if (isTRUE(plot)) {
    message("build_gimme() returns an object; plot it with plot_gimme(fit). ",
            "Ignoring plot = TRUE.")
  }
  ...
  stopifnot(is.numeric(groupcutoff), length(groupcutoff) == 1L,
            groupcutoff > 0, groupcutoff <= 1)
  ...
  if (isTRUE(subgroup)) {
    stop("build_gimme() does not implement subgrouping (S-GIMME); ",
  ...
  # so build_gimme takes the same id/day/beep interface as the other estimators.
  ...
  # and never cross the overnight gap -- matching graphical_var()/build_mlvar().
...
#' crossing the overnight gap, matching `graphical_var()`/`build_mlvar()`. With
...
.gimme_prepare_data <- function(data, vars, id, time, standardize, exogenous,
  ...
  ts_list <- lapply(ids, function(pid) {
    ...
    # Sort by time if provided (when day was supplied without time, build_gimme
    ...
    lag_mat <- mat[-n, , drop = FALSE]             # rows 1..n-1
...
.gimme_individual_search <- function(base_syntax, group_paths, data_k,
  ...
  while (!outer_done) {
    ...
    pruned_any <- FALSE
    if (converged && stable && length(ind_paths) > 0) {
      prune_done <- FALSE
      while (!prune_done) {
    ...
    exclude <- unique(c(dropped_param, nonconv_path))
...
.gimme_extract_results <- function(ind_results, varnames, lag_names,
  ...
  for (k in seq_len(n_subj)) {
    ...
    path_counts <- path_counts + (!is.na(mat) & mat != 0) * 1L
  }
