...
* New native Bayesian estimators that statistically reproduce Mplus DSEM output
  ...
  * `build_mlvar_bayes()` — two-level Bayesian VAR(1) with latent mean centering.
    `temporal = "fixed"` matches Mplus DSEM fixed temporal + random intercepts;
  ...
  * `build_var_bayes()` — single-level Bayesian VAR(1), the unregularized
    Bayesian analogue of `graphical_var()`.
...
* Validated to statistical (Monte-Carlo-error) equivalence against real Mplus 9
  output; frozen ground-truth fixtures and parity tests under
