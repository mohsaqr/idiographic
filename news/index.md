...
- The pure-R graphical-lasso kernel that powers
  ...
  [`glasso_fit()`](https://pak.dynasite.org/idiographic/reference/glasso_fit.md)
  returns a `glasso_result` whose `$wi` and `$w` elements are named
  exactly as
  [`glasso::glasso()`](https://rdrr.io/pkg/glasso/man/glasso.html)’s, so
  existing call sites port unchanged, and it supports element-wise
  penalty matrices and hard zero constraints in addition to a scalar
  penalty.
  [`glasso_kkt()`](https://pak.dynasite.org/idiographic/reference/glasso_kkt.md)
  certifies a solution from the graphical-lasso stationarity conditions
  rather than against another solver. Both results have
  [`print()`](https://rdrr.io/r/base/print.html) and tidy
  [`as.data.frame()`](https://rdrr.io/r/base/as.data.frame.html)
  methods. This exists so sibling packages can drop their own copies of
  the same kernel and depend on this one; see `OFFLOAD.md` in the
  repository.

- [`matrices()`](https://pak.dynasite.org/idiographic/reference/matrices.md)
  gains a `print` argument. The default (`print = TRUE`) is unchanged:
  it prints each matrix and returns the list invisibly. With
  `print = FALSE` the function prints nothing and returns the list
  visibly, which is the form dependent packages and resampling loops
  need. Threaded through every
  [`matrices()`](https://pak.dynasite.org/idiographic/reference/matrices.md)
  method, including those that delegate to another method. `print`
  follows `...` in every method, so it must be named in full and can
  never be matched positionally or by partial name.

### Bug fixes

- [`glasso_kkt()`](https://pak.dynasite.org/idiographic/reference/glasso_kkt.md)
  no longer reports optimal zero-constrained fits as non-optimal. At an
  entry hard-constrained by `zero`, the inactive-edge condition
  `|W_ij - S_ij| <= rho_ij` does not apply: the equality constraint
  carries its own Lagrange multiplier, which absorbs the residual.
  Constrained pairs are now excluded from the check. A fit matching
  `glasso`’s Fortran kernel to 2e-12 previously certified as violating
  optimality by 0.028.

- [`glasso_fit()`](https://pak.dynasite.org/idiographic/reference/glasso_fit.md)
  now rejects a covariance matrix that is not positive semi-definite
  (classed condition `idiographic_not_psd`). A negative eigenvalue
  produced a precision matrix with a negative diagonal, from which every
  partial correlation was `NaN`. Singular but positive semi-definite
  ...
  [`stats::cov2cor()`](https://rdrr.io/r/stats/cor.html) warn and emit
  `NaN` weights.

- An asymmetric element-wise penalty matrix is now rejected by
  [`glasso_fit()`](https://pak.dynasite.org/idiographic/reference/glasso_fit.md)
  and by
  [`fit_graphical_var()`](https://pak.dynasite.org/idiographic/reference/fit_graphical_var.md)’s
  `regularize_mat_kappa`. The penalty on edge `(i, j)` is a single
  scalar over a symmetric `Theta`, so an asymmetric penalty is ill-posed
  rather than stricter; previously it was accepted and produced a fit
  that failed the package’s own optimality check.

- [`glasso_kkt()`](https://pak.dynasite.org/idiographic/reference/glasso_kkt.md)
  validates `penalize_diagonal` instead of letting
  [`isTRUE()`](https://rdrr.io/r/base/Logic.html) silently map `NA` and
  other invalid values to `FALSE`, and warns
  (`idiographic_glasso_kkt_override`) when a supplied `rho` or
  `penalize_diagonal` differs from the one the fit was made with, since
  the returned violation then certifies a different problem.

- The internal graphical-lasso optimality checker used to measure the
  *unpenalised* diagonal stationarity condition (`W_ii = S_ii`) even for
  fits made with `penalize.diagonal = TRUE`, whose condition is
  `W_ii - S_ii = rho`. It therefore reported a spurious violation of
  exactly `rho` for every such fit — including `glasso`’s own Fortran
  output, which is how this was found. It now takes the diagonal-penalty
  flag into account. No estimator result and no previously published
  number changes; only the diagnostic was wrong.
